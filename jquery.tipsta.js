(function($) {
    $.fn.tipsta = function(options) {  
        var options = $.extend({}, $.fn.tipsta.defaults, options);
        ...
        function _createTip(id) {
			...
			return tt.addClass(options.class).appendTo(options.anchor);			
		}
		...
		function _calcPosition(elem, tip) {
			...
			if (_isImageMap(elem)) {
				...
				for (var i = 1, j = coords.length; i <= j; i++) {
			...
			for (var i = 0, j = attrs.length; i < j; i++) {
				y += _getNumeric(p.css(attrs[i] + "top"));
				x += _getNumeric(p.css(attrs[i] + "left"));
			}
			...
			x -= Math.round(tip.attr("offsetWidth") / 2);
			tip.css({ visibility: "visible", display: "none" });
			
			y += _getNumeric(tip.css("padding-top"));
			y += _getNumeric(tip.css("margin-top"));
        ...
        return this.each(function() {
			if ($(this).length == 0) {
				throw new Error("Cannot find " + elem + " in document");
			}
			...
			if (!tip.length) {
				tip = _createTip(
					id,
					_getCaption($(this), options.caption)
				);
			}
